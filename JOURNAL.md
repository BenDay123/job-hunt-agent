...
The bucketing pass left 10 rows in (or near) the Workday bucket at Medium confidence — the research agents had inferred Workday without seeing a `myworkdayjobs` URL directly. Ran a verification pass with direct search + page fetches. All 10 resolved:
...
- **Phillips 66 → SAP SuccessFactors.** Resolved via search rather than a fetch (the fetch was declined): the careers site exposes `careers.phillips66.com/go/{name}/{id}/` URLs — the SuccessFactors Career Site Builder pattern, the same signal used to identify the other 21 SuccessFactors companies. Moved out of Workday.

Net: Workday 129 → 126. Lesson for the rest of the buckets — the agents' Medium-confidence Workday calls were right 7 times out of 9, but the 2 misses were confident-looking and would have wasted v1 integration effort. Any Medium row that v1 depends on gets a direct page fetch before it's trusted.
...
**What's next.** The bigger open question still stands: decide whether Phenom/Avature get verified into the Workday bucket (could push the effective Workday count back up), and design the Workday discovery mechanism (tenant job-search API vs. per-tenant crawling).
